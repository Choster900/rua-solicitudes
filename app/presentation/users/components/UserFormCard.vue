        ...
        <form class="space-y-4 px-5 py-5" @submit.prevent="handleSubmit">
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-1.5">
                    ...
                    />
                    <p v-if="getFieldError('userType')" class="text-xs text-status-error">
                        {{ getFieldError('userType') }}
                    </p>
                </div>
...
const userTypeSelectOptions = userTypeOptions.map((userType) => ({
    label: userType,
    value: userType,
}))
...
watch(
    ...
    (nextUserType) => {
        if (
            props.mode !== 'create' ||
            !isEmployeeCodeAutoManaged.value ||
            !props.resolveEmployeeCode
        ) {
            return
        }
