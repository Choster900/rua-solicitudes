...
export const useUsersModule = () => {
  ...
  const generateNextEmployeeCode = (userType: UserFormModel['userType'] = '') => {
    ...
    const highestSequence = users.value.reduce((currentHighest, user) => {
  ...
  const createUser = async (formModel: UserFormModel) => {
    const resolvedEmployeeCode = formModel.employeeCode.trim() || generateNextEmployeeCode(formModel.userType)
  ...
  const updateUser = async (userId: string, formModel: UserFormModel) => {
    ...
    const duplicatedEmployeeCode = users.value.some((user) => {
      return user.id !== userId && user.employeeCode.trim().toLowerCase() === normalizedEmployeeCode
    })
  ...
  const sendTemporaryAccess = async (userId: string) => {
    ...
    try {
      ...
      const httpError = error as HttpClientError
      const statusMessage = (httpError.details as { statusMessage?: string } | null)?.statusMessage
      toast.error(statusMessage || httpError.message || 'No se pudo actualizar el acceso temporal.')
    }
  }
