<template>
    <aside :class="sidebarClass">
        <div
            :class="['mb-stack-lg flex items-center justify-center', isCollapsed ? 'px-3' : 'px-6']"
        >
            <img
                alt="RUASA Logo"
                :class="[
        ...
        <nav :class="['flex-1 space-y-2', isCollapsed ? 'px-2' : 'px-4']">
            ...
            >
                ...
                <span
                    ...
                    :class="[item.key === activeItemKey ? 'fill-icon' : '', 'sidebar-icon-orange']"
                >
                    {{ item.icon }}
                </span>
                <span v-if="!isCollapsed" class="font-label-caps text-label-caps text-white">
                    {{ item.label }}
                </span>
            </button>
        </nav>
        ...
        <div
            :class="[
                'mt-stack-md border-t border-outline/20 pt-stack-md',
        ...
        >
            <button
                ...
                @click="$emit('logout')"
            >
                <span class="material-symbols-outlined sidebar-icon-orange"> logout </span>
                <span v-if="!isCollapsed" class="font-label-caps text-label-caps text-white">
                    Cerrar sesión
                </span>
            </button>
...
const allNavigationItems: AppShellNavItem[] = [
    ...
    {
        key: 'bandeja-diseno',
        label: 'Bandeja Diseño',
        icon: 'design_services',
        allowedRoles: ['admin', 'disenador_jefe', 'disenador'],
    },
    {
        key: 'bandeja-calidad',
...
const navigationItems = computed(() => {
    ...
    return allNavigationItems.filter((item) => {
        ...
        return item.allowedRoles.some((role) => authTokenHasRole(token, role))
    })
})
...
const sidebarClass = computed(() => {
    ...
    const translateClass =
        props.isMobileViewport && !props.isMobileSidebarOpen ? '-translate-x-full' : 'translate-x-0'
...
.sidebar-icon-orange {
    color: #f59e0b;
    filter: brightness(0) saturate(100%) invert(66%) sepia(79%) saturate(1063%) hue-rotate(358deg)
