    ...
    >
        <template #cell-requestCode="{ row }">
            ...
            <button
                ...
                type="button"
                @click.stop="handleView((row as any).id)"
            >
                <span class="material-symbols-outlined text-[16px]">open_in_new</span>
                {{ (row as any).requestCode }}
            </button>
        </template>

        <template #cell-priorityLabel="{ row }">
            <AppStatusBadge
                :label="(row as any).priorityLabel"
                :tone="resolvePriorityToneByRow((row as any).id)"
            />
        </template>

        <template #cell-stageLabel="{ row }">
            <AppStatusBadge
                :label="(row as any).stageLabel"
                :tone="resolveStageToneByRow((row as any).id)"
            />
        </template>
...
const resolveStageToneByRow = (requestId: string): AppStatusBadgeTone => {
    ...
    return props.stageToneMap[stage] ?? 'neutral'
}
...
const resolvePriorityToneByRow = (requestId: string): AppStatusBadgeTone => {
    ...
    return props.priorityToneMap[priority] ?? 'neutral'
}
</script>
