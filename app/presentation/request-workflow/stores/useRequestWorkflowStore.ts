...
export const useRequestWorkflowStore = defineStore('request-workflow', {
    ...
    getters: {
        ...
        requestById: (state) => (requestId: string) =>
        ...
        approvedRequests: (state) =>
            state.requests.filter((request) => request.stage === 'APPROVED'),
        metrics: (state) => {
            const total = state.requests.length
            const designPending = state.requests.filter((request) =>
                designQueueStages.includes(request.stage),
            ).length
            const qualityPending = state.requests.filter((request) =>
    ...
    actions: {
        ...
        upsertFromDesignRequest(request: DesignRequest) {
            ...
            if (existingRequest) {
                const hasChanges =
                    ...
                    existingRequest.stage !== sharedPayload.stage ||
        ...
        rejectInQuality(
            ...
            const appendResult = this.appendObservation(
                requestId,
                observation,
                actorName,
                'Calidad',
        ...
        transitionStage(payload: {
                ...
                auditTrail: [
                    {
                        ...
                        comment:
                            ...
                            `${stageLabelMap[previousStage]} -> ${stageLabelMap[payload.nextStage]}`,
                        createdAt: nowIso,
                    },
                    ...target.auditTrail,
                ],
            ...
            this.requests = this.requests.map((request) =>
                request.id === payload.requestId ? updatedRequest : request,
            )
