...
import { findDesignRequestByCode, findDesignRequestById, updateDesignRequest } from '../../repositories/design-requests.repository'
...
export default defineEventHandler(async (event) => {
  ...
  const finishingOptions = Array.isArray(body.finishingOptions) ? body.finishingOptions : sourceRequest.finishingOptions
  const deliverables = Array.isArray(body.deliverables) ? body.deliverables : sourceRequest.deliverables
  ...
  const requireDieCut = typeof body.requireDieCut === 'boolean' ? body.requireDieCut : sourceRequest.requireDieCut
  const requireMockup = typeof body.requireMockup === 'boolean' ? body.requireMockup : sourceRequest.requireMockup
  const attachments = Array.isArray(body.attachments) ? body.attachments : sourceRequest.attachments
